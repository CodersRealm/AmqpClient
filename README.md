...
```bash
dotnet add package CodersRealm.AmqpClient
```
...
builder.Services.AddAmqpClient("amqp://localhost:5672?username=admin&password=admin");
...
```
// Standard connection
amqp[s]://{Host}:{Port}?username={UserName}&password={Password}

// Example:
amqps://localhost:5672?username=admin&password=admin

// Failover and automatic reconnection
failover:(amqp[s]://{Host}:{Port}[, amqp[s]://{Host}:{Port}, ...])?username={UserName}&password={Password}

//Example:
failover:(amqps://host1:5672,amqps://host2:5672)?username=admin&password=admin

```

### Azure Service Bus with Shared Access Key
```
amqps://{Namespace}.servicebus.windows.net?username={SasPolicyName}&password={UrlEncodedSasKeyValue}
```
...
### Basic Setup
```csharp
builder.Services.AddAmqpClient("amqp://localhost:5672");
```
